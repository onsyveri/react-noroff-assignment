...
const TranslationForm = ({ onTranslation }) => {
  ...
  const onSubmit = (data) => {
    onTranslation(data.translationField);
    ...
    for (let i = 0; i < letterArr.length; i++) {
      for (let y = 0; y < 26; y++) {
        if (letterArr[i] === IMAGES[y].id) {
          // Compare letter from input word to the alphabet
          let OBJ = {};
          OBJ.id = i;
    ...
    setHandImage(
      TEST.map((signImage) => {
        // Maps the new array and sets it as the handImage using useState()
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <fieldset>
          <input
            ...
            placeholder="Type in word you want to translate..."
          />
        </fieldset>

        <button id="btnTranslate" type="submit">
          Translate
        </button>
